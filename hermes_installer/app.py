...
class InstallerWindow(QWidget):
    ...
    def _install_finished(self, ok: bool, message: str) -> None:
        ...
        if ok:
            self.status_label.setText("Install complete. Open a setup terminal next.")
            self.setup_button.setEnabled(True)
            self.launch_button.setEnabled(True)
            QMessageBox.information(
                self,
                "Hermes installed",
                "Hermes was installed successfully.\n\nUse 'Open Setup Terminal' to run the upstream setup wizard.",
            )
            return
    ...
    def _open_setup_terminal(self) -> None:
        options = self._current_options()
        self.installer.open_terminal_for_setup(options)
